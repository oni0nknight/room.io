...
})
{
    ...
    this.run = ({ port = SERVER_PORT, httpServer = null } = {}) =>
    {
        ...
        this.io.on('connection', (socket) =>
        {
            ...
            // Custom actions
            actions.forEach(({ name, inputValidator }) =>
            {
                socket.on(name, customAction(socket, name, inputValidator))
            })
    ...
    {
        this.logger.info(`requesting to join a room the room with code ${data ? data.code : null}`, { socket: socket.id })
    ...
    const startGame = (socket) => () =>
    {
        ...
        const formattedPlayers = formatPlayers(playerRoom.players, ['publicId', 'name', 'data'])
        if (!roomSettingsChecker(playerRoom.settings, formattedPlayers))
        {
    ...
    const customAction = (socket, name, inputValidator) => (data) =>
    {
