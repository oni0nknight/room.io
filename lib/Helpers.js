...
const Helpers = {
    ...
    sendError(socket, error, args = {}) {
        Logger.error(socket.id, 'ERROR => ' + error.debug_msg)
     ...
     * @emits reconnection event sent to the player in the case of a reconnection
    ...
    connect(socket) {
        ...
        if (oldPlayer && oldPlayer.online === false && oldPlayer.roomId !== null && rooms[oldPlayer.roomId])
        {
            ...
            delete players[socket.handshake.query.playerID]
     ...
     * @emits playersUpdated event sent to all the remaining players in the room, if the game instance is not started
     */
    leaveRoom(playerId, removePlayer) {
        ...
        if (room.gameInstance !== null)
        {
            ...
            if (stillOnlinePlayers.length)
            {
                ...
                io.to(roomId).emit('playerLeft', player.publicId)
            }
